...
def analyze_risk_control(data: dict, module_results: dict | None = None) -> ModuleResult:
    ...
    if current:
        ...
        findings.append(f"当前价格：{current:.2f}")
        ...
        if risk > 0:
            ...
            findings.append(f"📊 风险收益比（盈亏比）：{rr_ratio:.2f}:1")
            if rr_ratio >= 2.0:
                score += 1.5
    ...
    if total_score:
        ...
        findings.append(f"📦 仓位建议：{position_advice}")
    else:
    ...
    return ModuleResult(
        ...
        detail={"ma5": ma5, "ma10": ma10, "ma20": ma20, "ma30": ma30},
    )
